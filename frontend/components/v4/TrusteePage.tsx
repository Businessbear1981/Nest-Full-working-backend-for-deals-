...
const STATUS_COLOR: Record<string, string> = {
  pending: "border-[#2D6B3D] text-[#7A9A82]",
      ...
      <section className="relative overflow-hidden rounded-[1.8rem] border border-[#C4A048]/25 bg-[#060E1A] p-5 text-[#EDE8DC] shadow-[0_0_85px_rgba(196,160,72,0.11)] sm:p-7">
        ...
        <div className="relative flex items-start justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-[#EDE8DC]">Trustee selection, fee benchmarking, performance tracking, document coordination, and reporting. Every bond has a trustee — this desk manages those relationships.</p>
          </div>
          {tasks.length > 0 && (
            <div className="text-right">
              <p className="font-mono text-2xl text-emerald-400">{completedCount}/{tasks.length}</p>
              <p className="font-mono text-[0.6rem] uppercase text-[#7A9A82]">Tasks Complete</p>
            </div>
          )}
        </div>
      </section>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="providers" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {TRUSTEE_BANKS.map((t) => (
              <Card key={t.name} className="border-[#1E4A2E] bg-[#0D2218] hover:border-[#C4A048]/40 transition">
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between">
                    <CardTitle className="text-sm text-white">{t.name}</CardTitle>
                    <Badge variant="outline" className={t.tier === "dominant" ? "border-[#C4A048]/40 text-[#C4A048]" : t.tier === "major" ? "border-emerald-400/40 text-emerald-300" : "border-[#2D6B3D] text-[#7A9A82]"}>{t.tier}</Badge>
                  </div>
                </CardHeader>
                <CardContent className="space-y-2">
                  <p className="text-xs text-[#7A9A82]">{t.specialty}</p>
                  <p className="font-mono text-sm text-[#C4A048]">{t.fee_range}</p>
                  <div className="flex flex-wrap gap-1 mt-2">
                    {t.strengths.map((s) => <span key={s} className="text-[0.6rem] bg-white/5 border border-white/10 rounded px-1.5 py-0.5 text-[#7A9A82]">{s}</span>)}
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        <TabsContent value="tasks" className="mt-6">
          <div className="flex gap-2 mb-4">
            {["", "pre-issuance", "post-issuance", "ongoing"].map(p => (
              <Button key={p} size="sm" variant={phaseFilter === p ? "default" : "outline"}
                className={phaseFilter === p ? "bg-cyan-700 text-white" : "border-[#1E4A2E] text-[#7A9A82]"}
          ...
          {tasksLoading ? (
            <p className="text-sm text-[#7A9A82] animate-pulse">Loading tasks...</p>
          ) : tasks.length === 0 ? (
            <Card className="border-[#1E4A2E] bg-[#0D2218]"><CardContent className="p-6 text-center text-[#7A9A82] text-sm">No tasks loaded. Tasks are seeded per deal when first accessed via /api/trustee/deals/&lt;deal_id&gt;/tasks.</CardContent></Card>
          ) : (
              ...
              <div key={phase} className="mb-6">
                ...
                <div className="space-y-2">
                  {(phaseTasks as any[]).map(task => (
                    <Card key={task.id} className="border-[#1E4A2E] bg-[#0D2218]">
                      <CardContent className="p-4 flex items-center justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-semibold text-white">{task.task_label}</span>
                            <Badge variant="outline" className={STATUS_COLOR[task.status] || "border-[#2D6B3D] text-[#7A9A82]"}>{task.status.replace("_", " ")}</Badge>
                          </div>
                          {task.assignee && <p className="text-xs text-[#7A9A82] mt-0.5">Assigned: {task.assignee}</p>}
                          {task.due_date && <p className="text-xs text-[#7A9A82]">Due: {task.due_date}</p>}
                        </div>
        ...
        <TabsContent value="fees" className="mt-6">
          <Card className="border-[#1E4A2E] bg-[#0D2218]"><CardContent className="p-6">
            <p className="text-sm text-[#EDE8DC]">Fee benchmarking module — compare trustee fees across providers by deal size, complexity, and sector. Coming soon: live fee quotes from trustee bank APIs.</p>
          </CardContent></Card>
        </TabsContent>
        <TabsContent value="performance" className="mt-6">
          <Card className="border-[#1E4A2E] bg-[#0D2218]"><CardContent className="p-6">
            <p className="text-sm text-[#EDE8DC]">Trustee performance tracking — response times, reporting quality, error rates, relationship health scores. Populated as deals enter administration.</p>
          </CardContent></Card>
        </TabsContent>
      </Tabs>
    </div>
