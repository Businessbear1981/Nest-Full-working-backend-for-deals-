...
export function PodCodeIntakeForm() {
  ...
  return (
    <Card className="border-[#C4A048]/25 bg-[#06111c]/90 p-5 text-[#EDE8DC] shadow-[0_0_42px_rgba(196,160,72,0.10)]">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <div className="space-y-4 rounded-2xl border border-white/10 bg-black/25 p-4">
          <label className="block text-sm font-semibold text-[#EDE8DC]">
            Sponsor
            <input
              value={sponsor}
              onChange={(event) => setSponsor(event.target.value)}
              className="mt-2 w-full rounded-xl border border-[#1E4A2E] bg-black px-3 py-2 text-sm text-white outline-none ring-cyan-300/40 focus:ring-2"
            />
          </label>

          <div>
            <p className="mb-2 flex items-center gap-2 text-sm font-semibold text-[#EDE8DC]"><Building2 size={15} /> Asset class</p>
            <div className="flex flex-wrap gap-2">
              {assetClasses.map((item) => (
                <Button key={item} type="button" size="sm" variant={assetClass === item ? "default" : "outline"} onClick={() => setAssetClass(item)} className={assetClass === item ? "bg-cyan-600 text-white hover:bg-cyan-500" : "border-[#1E4A2E] bg-black/20 text-[#EDE8DC]"}>
                  {item}
                </Button>
              ))}
            </div>
          ...
          <div>
            <p className="mb-2 flex items-center gap-2 text-sm font-semibold text-[#EDE8DC]"><MapPin size={15} /> Jurisdiction</p>
            <div className="flex flex-wrap gap-2">
              {jurisdictions.map((item) => (
                <Button key={item} type="button" size="sm" variant={jurisdiction === item ? "default" : "outline"} onClick={() => setJurisdiction(item)} className={jurisdiction === item ? "bg-cyan-600 text-white hover:bg-cyan-500" : "border-[#1E4A2E] bg-black/20 text-[#EDE8DC]"}>
                  {item}
                </Button>
              ))}
            </div>
        ...
        <div className="rounded-2xl border border-[#C4A048]/20 bg-black/30 p-5">
          <p className="font-mono text-[0.68rem] uppercase tracking-[0.16em] text-[#7A9A82]">Generated pod code</p>
          ...
          <div className="mt-5">
            <p className="mb-3 font-mono text-[0.68rem] uppercase tracking-[0.16em] text-[#7A9A82]">Auto-populated permit checklist</p>
              ...
              {permits.map((permit, index) => (
                <div key={permit} className="flex items-start justify-between gap-3 rounded-xl border border-white/10 bg-white/[0.035] p-3">
                  <div className="flex items-start gap-2">
                    <ShieldCheck size={16} className={generated ? "mt-0.5 text-emerald-300" : "mt-0.5 text-[#7A9A82]"} />
                    <div>
                      <p className="text-sm font-medium text-white">{permit}</p>
                      <p className="text-xs text-[#7A9A82]">Checklist item {index + 1} · evidence routed to Roots</p>
                    </div>
                  </div>
                  <Badge variant="outline" className={generated ? "border-emerald-300/35 bg-emerald-400/10 text-emerald-100" : "border-[#2D6B3D]/35 bg-[#2D6B3D]/10 text-[#EDE8DC]"}>{generated ? "READY" : "DRAFT"}</Badge>
                </div>
              ))}
            </div>
          </div>
