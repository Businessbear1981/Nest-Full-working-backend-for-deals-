...
9. **Coordinated/pooled offering ("Rico," built this session).** If Eagle Eye's `find_comparable_deals()` finds 3+ other real deals with similar LTV/DSCR/leverage/rating in the same sector, `EagleEyeScanner.structure_pooled_offering()` now turns that cohort into a real structured coordinated offering: par-weighted DSCR/LTV/leverage (not a naive average), a weakest-link blended credit rating (no speculative pooling-uplift notching — same discipline as refusing to invent an `ENHANCEMENT_RATING_MAP` uplift), and real arrangement-fee economics. **If** 3 CCRC deals pool $150M total par at NEST's real 2.25% arrangement-fee rate, **then** the pooled fee ($3.375M) equals the exact sum of what each deal would pay standalone — pooling is never used to justify a higher fee percentage (this was the Ticket 16 PhaseBondEngine lesson, applied proactively here). The real, legitimate case for pooling is fewer independent transactions (one rating submission, one enhancement negotiation, one legal/diligence engagement instead of N), and shared bond-insurance pricing via `bond_insurance_premium()` when the pool has real aggregate debt-service data. Below 3 deals, or with no real par data, it correctly returns `viable: False` rather than fabricating a pool. 12 regression tests in `backend/tests/test_pooled_offering.py`.

**The one-sentence version:** signals get found and scored for real, sizing and underwriting math is real and (as of this session) more honest about its own limits, and the pooled-offering structuring layer is now real too — but the bond-type-selection engine and the real buyer list remain the two biggest disconnects between "the math is right" and "the platform actually places a bond." (One newly confirmed gap, not yet built anywhere: appraisal analysis — see §7.)

**10. Fake-success rollup (asked directly this session, answer belongs here so it doesn't have to be re-derived).** Of the 147 catalogued backend files, **21 return a fabricated result presented as a real success** (full list and per-file detail in §8's module inventory tables):
- **16 are silent** — 200/success responses with fake data and no indication it isn't real: `services/treasury_engine.py` (worst — the entire live "Treasury" feature is 100% seeded fake data, no real path exists at all), `routes/construction.py` (returns the same fake deal for *any* real `deal_id` requested), `services/emma_seed_data.py` (docstring mislabels its fake bonds "real"), `services/deals.py` + `routes/deals.py` + `routes/intake_brainstorm.py` (three conflicting fabricated dollar figures for the one real "Jacaranda Trace" deal — $42M / $205M / self-contradicting $205M-and-$231M), `routes/bond_tools.py`, `routes/covenants.py`, `routes/emma.py`, `routes/hawkeye.py`, `routes/treasury.py`, `routes/insurance.py`, `services/roots_service.py`, `services/fund_engine.py`, `services/core.py` (buried in `HFTEngine.simulate()`), `agents/lender_scout.py`, `agents/sterling.py`.
- **5 are labeled/transparent** — still fake data, but honestly tagged so it's distinguishable from real: `services/data_connectors.py`, `routes/investors.py`, `routes/surveillance.py`, `agents/vector_agent.py`, `sendgrid_service.py`.
- **2 already fixed this session, no longer counted above:** `ConvergenceEngine`, `MerlinAgent` — used to auto-fabricate demo entities in production, now don't.
...
- **Also lives here:** **Rico** *(built this session)* — coordinated/pooled multi-deal offering structuring, for when Eagle Eye's `find_comparable_deals()` (§5) surfaces 3+ real comparable deals worth placing together instead of one at a time. Cohort-matching (`find_comparable_deals()`) plus the structuring layer itself (`structure_pooled_offering()`) are both real now — par-weighted DSCR/LTV/leverage, weakest-link blended rating, fee-parity vs. standalone (12 regression tests, `backend/tests/test_pooled_offering.py`). See §4 for the full entry.
...
- **Actual (verified):** `scan_for_equity_partners()` (Ticket 11) and `find_comparable_deals()` (Ticket 22 partial, this session) both added — real gap-detection and cohort-matching math, deliberately returning empty results rather than fabricating counterparties/comparable deals when no real roster/candidate data exists (mirrors the Hawkeye fix below). **`structure_pooled_offering()` — "Rico," built this session** — takes `find_comparable_deals()`'s cohort (3+ real matched deals) and structures it into one coordinated offering: par-weighted DSCR/LTV/leverage (real weighted average, not naive), weakest-link blended credit rating (no invented pooling-uplift), and real arrangement-fee economics — the pooled fee is always exactly the sum of what each deal would pay standalone at NEST's real 2.25% rate, deliberately never inflated to justify pooling (the Ticket 16 PhaseBondEngine conflict-of-interest lesson, applied proactively rather than found as a bug). Shared bond-insurance pricing via `bond_insurance_premium()` when the pool has real aggregate debt-service data (explicit `annual_debt_service`, or derived from `noi/dscr` — never approximated from par, which has no fixed relationship to debt service). Returns `viable: False` below 3 deals or with no real par data, rather than fabricating a pool. 12 regression tests in `backend/tests/test_pooled_offering.py`.
- **Fix needed:** none remaining from this session's scope. Downstream, the *appraisal analysis* piece of Ticket 22's full CRE intelligence expansion is still unbuilt — see §7.
...
- **No appraisal analysis engine exists anywhere in the platform** (confirmed this session, asked directly). "Appraisal" appears in exactly three places, all superficial: `services/due_diligence.py` line 73 lists "Property appraisals (MAI)" as a required-document checklist item (a checkbox, nothing parsed); `services/doc_ingestion.py` lists "appraisal" as one of the document *types* its classifier can tag a file as (it identifies that a doc is an appraisal, doesn't read it); `agents/morgan.py` line 135 mentions it in a prompt string listing documents to reference. **Nothing extracts an appraised value, cap rate, or comparable-sales analysis from an appraisal document and feeds it into DSCR/LTV/structuring math** — this was already flagged as part of Ticket 22's unbuilt CRE intelligence expansion (§5, Eagle Eye) but is worth calling out on its own: it's a real gap, not a wiring or naming issue like Marshal/Rico were.
